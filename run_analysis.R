...
#Read the datafiles into R - Combine the data, subjects and activity rows
...
#print(paste("Imported", nrow(subjects),"subject entries" ))
...
datacolnames <- datacolnames[complete.cases(datacolnames),] # This clears the empty NA value and converts to character vector to be used to name the data columns lateru
...
# Objective 3 - Add descriptive activity names by replacing the values in the second "Activity" column
  ...
  print ("Objective 3 Completed - Descriptive activity names assigned")
...
# Objective 4 - Name the variables
  ...
  colnames(filtered_data)<- gsub("Acc", " Accelerometer", names(filtered_data))
...
# Objective 5 - Tidy data set with average for each subject and activity
  ...
  # Write output file
  write.table(AverageResult, file="tiydataset.txt", row.names = FALSE)
